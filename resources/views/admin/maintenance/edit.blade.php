        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-6 py-5 border-b border-gray-100">
                    <h3 class="text-lg font-bold text-gray-800">Edit Maintenance Record</h3>
                    <p class="text-sm text-gray-500">Update maintenance information</p>
                </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                ...
                                <select name="machine_id" class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500">
                                    @foreach($machines as $machine)
                                        <option value="{{ $machine->machine_id }}" {{ old('machine_id', $maintenance->machine_id) == $machine->machine_id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="reported_by" class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500">
                                    @foreach($employees as $employee)
                                        <option value="{{ $employee->employee_id }}" {{ old('reported_by', $maintenance->reported_by) == $employee->employee_id ? 'selected' : '' }}>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="description" rows="4"
                                          class="w-full rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500">{{ old('description', $maintenance->description) }}</textarea>
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-600 mb-1">Report Date</label>
                                <input type="date" name="report_date" value="{{ old('report_date', $maintenance->report_date) }}"
